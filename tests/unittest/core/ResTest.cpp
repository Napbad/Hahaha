...
class TestError final : public Error
{
  ...
  public:
    explicit TestError(std::string message, std::string location = "test")
        : msg_(std::move(message)), loc_(std::move(location))
    {
    }
    ...
    {
        return msg_ == other.msg_ && loc_ == other.loc_;
    }
};
...
TEST_F(ResTest, ErrConstruction)
{
    ...
    EXPECT_EQ(r1.unwrapErr().message().cStr(), std::string("error1"));
    ...
    EXPECT_EQ(r2.unwrapErr().message().cStr(), std::string("error2"));
    ...
    EXPECT_EQ(r3.unwrapErr().message().cStr(), std::string("error3"));
    ...
    EXPECT_EQ(r4.unwrapErr().message().cStr(), std::string("error4"));
}
...
TEST_F(ResTest, UnwrapErr)
{
    ...
    EXPECT_EQ(err.unwrapErr().message().cStr(), std::string("error"));

    // Test const unwrapErr
    const Res<int, TestError>& constErr = err;
    EXPECT_EQ(constErr.unwrapErr().message().cStr(), std::string("error"));

    // Test move unwrapErr (should consume the error)
    TestError movedErr = std::move(err).unwrapErr();
    EXPECT_EQ(movedErr.message().cStr(), std::string("error"));
}
...
TEST_F(ResTest, Map)
{
    ...
    EXPECT_EQ(mappedErr.unwrapErr().message().cStr(), std::string("error"));
