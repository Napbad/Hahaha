...
protected:
    ...
    bool rootIsBlack() {
        if (!tree.getRoot()) return true; // An empty tree is valid
        return tree.getRoot()->isBlack();
    }

    // Property 3 (all leaves are black) is implicit as nullptr is considered black.
    ...
    bool noRedRedViolation(RBTreeNode<T>* node) {
        ...
        return noRedRedViolation(node->getLeft()) && noRedRedViolation(node->getRight());
    }

    // Property 5 (same black height on all paths)
    int blackHeight(RBTreeNode<T>* node) {
        if (!node) return 1; // Leaf nodes contribute 1 to black height
    ...
    void verifyProperties() {
        ...
        ASSERT_TRUE(isBST(tree.getRoot(), std::numeric_limits<T>::min(), std::numeric_limits<T>::max()));
    }
};
