...
template <typename T> class AVLTreeTest : public ::testing::Test
{
  protected:
    AVLTree<T> tree;
    ...
    bool verifyNodeHeights(AVLTreeNode<T>* node)
    {
        ...
        return verifyNodeHeights(node->getLeft())
            && verifyNodeHeights(node->getRight());
    }
    ...
    bool verifyBalanceFactor(AVLTreeNode<T>* node)
    {
        ...
        return verifyBalanceFactor(node->getLeft())
            && verifyBalanceFactor(node->getRight());
    }
