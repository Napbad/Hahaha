...
namespace hahaha::test {
    ...
    TEST_F(ResTest, MapErr) {
        ...
        // MapErr on Ok value
        auto mappedOk =
            ok.mapErr([](const TestError& e) { return TestError(std::string(e.message().data()) + "-mapped"); });
        EXPECT_TRUE(mappedOk.isOk());
        EXPECT_EQ(mappedOk.unwrap(), 42);

        // MapErr on Err value
        auto mappedErr =
            err.mapErr([](const TestError& e) { return TestError(std::string(e.message().data()) + "-mapped"); });
        EXPECT_TRUE(mappedErr.isErr());
        EXPECT_EQ(mappedErr.unwrapErr().message().data(), std::string("error-mapped"));

        // Move semantics with mapErr
        auto movedMap =
            std::move(err).mapErr([](TestError&& e) { return TestError(std::string(e.message().data()) + "-moved"); });
