...
TEST_F(CSVDatasetTest, Constructor) {
    ...
    EXPECT_EQ(dataset.description(), ds::Str("Test dataset"));
}
...
TEST_F(CSVDatasetTest, FeatureAndLabelNames) {
    ...
    EXPECT_EQ(featureNames[0], ds::Str("feature1"));
    EXPECT_EQ(featureNames[1], ds::Str("feature2"));
    EXPECT_EQ(labelNames[0], ds::Str("label"));
}
...
TEST(DatasetErrorTest, ErrorHandling) {
    const DatasetError error(ds::Str("Test error"), ds::Str("test_location"));

    EXPECT_EQ(error.typeName(), ds::Str("DatasetError"));
    EXPECT_EQ(error.message(), ds::Str("Test error"));
    EXPECT_EQ(error.location(), ds::Str("test_location"));
    EXPECT_EQ(error.toString(), ds::Str("DatasetError: Test error at test_location"));
}

TEST(CSVDatasetErrorTest, ErrorHandling) {
    const CSVDatasetError error(ds::Str("CSV Test error"), ds::Str("csv_test_location"));

    EXPECT_EQ(error.typeName(), ds::Str("CSVDatasetError"));
    EXPECT_EQ(error.message(), ds::Str("CSV Test error"));
    EXPECT_EQ(error.location(), ds::Str("csv_test_location"));
    EXPECT_EQ(error.toString(), ds::Str("CSVDatasetError: CSV Test error at csv_test_location"));
}
