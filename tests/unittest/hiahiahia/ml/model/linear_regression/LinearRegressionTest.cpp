...
TEST_F(LinearRegressionTest, BasicPropertiesTest) {
    ...
    // Test model name
    EXPECT_EQ(lr.modelName(), Str("LinearRegression"));
...
TEST_F(LinearRegressionTest, TrainSimple2DTest) {
    ...
    // Train the model
    auto result = lr.train(features_2d, labels_2d);
    ...
    f32 pred1 = lr.predict(test_feature);
...
TEST_F(LinearRegressionTest, CheckStatusTest) {
    ...
    auto status = lr.checkStatus(features_2d, labels_2d);
...
TEST_F(LinearRegressionTest, SaveLoadTest) {
    ...
    EXPECT_TRUE(lr.save(Str("test_model.bin")));
    EXPECT_TRUE(lr.load(Str("test_model.bin")));
}
