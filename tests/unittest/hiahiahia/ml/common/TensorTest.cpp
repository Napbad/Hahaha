...
TYPED_TEST(TensorTest, CopyFromVector) {
    ...
    vec.push_back(static_cast<TypeParam>(2));
    vec.push_back(static_cast<TypeParam>(3));
    vec.push_back(static_cast<TypeParam>(4));
...
TEST_F(TensorFloatTest, OperatorParenthesesAccess) {
    Tensor<f32> tensor({3, 3});
    ds::Vec<sizeT> idx1{0, 0};
    ds::Vec<sizeT> idx2{1, 2};
    ds::Vec<sizeT> idx3{2, 2};
...
// Test copy with Vec size mismatch
...
TEST_F(TensorFloatTest, FromVectorLarge) {
    ...
    for (sizeT i = 0; i < 100; ++i) {
        vec.push_back(static_cast<f32>(i));
    }
