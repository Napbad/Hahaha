...
TEST_F(TensorTest, MoveSemantics) {
    TensorWrapper<int> t1({{1, 2}, {3, 4}});
    TensorWrapper<int> t2(std::move(t1));
...
TEST_F(TensorTest, HigherDimensionalTensor) {
    TensorWrapper<int> t({{{1, 2}, {3, 4}}, {{5, 6}, {7, 8}}});
...
TEST_F(TensorTest, ArithmeticOperations) {
    TensorWrapper<float> t1({{1.0f, 2.0f}, {3.0f, 4.0f}});
    TensorWrapper<float> t2({{5.0f, 6.0f}, {7.0f, 8.0f}});
...
TEST_F(TensorTest, Reshape) {
    TensorWrapper<int> t({1, 2, 3, 4, 5, 6});
...
TEST_F(TensorTest, MatrixMultiplication) {
    TensorWrapper<float> A({{1.0f, 2.0f}, {3.0f, 4.0f}}); // 2x2
    TensorWrapper<float> B({{5.0f, 6.0f}, {7.0f, 8.0f}}); // 2x2
...
TEST_F(TensorTest, Transpose) {
    TensorWrapper<int> t({{1, 2, 3}, {4, 5, 6}}); // 2x3
