...
template <typename T, typename E> class Res
{
  public:
    ...
    static Res err(E&& error)
    {
        return Res(std::forward<E>(error));
    }
    static Res err(const E& error)
    {
        return Res(error);
    }
    static Res err(std::unique_ptr<E>&& error)
    {
        return Res(std::move(error));
    }
    static Res err(E* error)
    {
        return Res(error);
    ...
    {
        return std::holds_alternative<std::unique_ptr<E>>(data);
    }
    ...
    E& unwrapErr() &
    {
        if (!isErr())
        {
            throw std::runtime_error("Called unwrapErr() on an Ok value");
        }
