...
{
  public:
    template <typename T> Res<Tensor<T>, ComputeErr> add(Tensor<T> src1, Tensor<T> src2)
    {
        SetRetT(Tensor<T>, ComputeErr) if (src1.shape() != src2.shape()){
            Err("the src1's shape is not same to the src2!")} Ok(src1 + src2);
    }

    template <typename T> Res<Tensor<T>, ComputeErr> mul(Tensor<T> src1, Tensor<T> src2)
    {
        SetRetT(Tensor<T>, ComputeErr) if (src1.shape() != src2.shape()){
            Err("the src1's shape is not same to the src2!")} Ok(src1 * src2);
    }

    template <typename T> Res<Tensor<T>, ComputeErr> sub(Tensor<T> src1, Tensor<T> src2)
    {
        SetRetT(Tensor<T>, ComputeErr) if (src1.shape() != src2.shape()){
            Err("the src1's shape is not same to the src2!")} Ok(src1 - src2);
    }
